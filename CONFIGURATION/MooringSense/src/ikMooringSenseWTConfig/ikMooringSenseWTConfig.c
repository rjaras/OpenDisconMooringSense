...
/**
 * @file ikMooringSenseWTConfig.c
 *
 * @brief MooringSense wind turbine controller configuration implementation
 */
...
void setParams(ikMooringSenseWTConParams *param, double samplingInterval) {
    /*! [Sampling interval] */
    /*
      ####################################################################
      Sampling interval
    ...
    */
    ...
    ikTuneThurstLimitter(&(param->thurstLimiter), &(param->tsrEstimator), samplingInterval);
}
...
void ikTuneTorqueFromHubPitch(ikTfListParams *params, double T) {
    ...
    /*! [Torque from nacelle pitch transfer function] */
    /*
      ####################################################################
    ...
    */
    ...
    //const double Kev = 0.0; /* DEACTIVATED */
...
void ikTuneThurstLimitter(ikThrustLimParams* thurstLimParams, ikTsrEstParams* tsrEstParams, double T) {
    ...
    /*
      ...
      Set parameters here:
    */
    const double b = 50.0; /* [-] - gearbox ratio HSS/LSS */
    const double J = 160099282.208; /* [kg m^2] rotor moment of inertia */
    const double R = 89.2; /* [m] - rotor radius */
    const double rho = 1.225; /* [kg/m^3] - air density */

    double cplambda3Max = 0.011;
    double cplambda3Min = -0.002;
    double tipSpeedRatioMax = 25.0;

    double ctlambda2Max = 0.04;
    double ctlambda2Min = -0.01;
    double minPitchMaxChangeRate = 15.0; /* [deg/s] */
    ...
    */
    ...
    //const char* ctlambda2SurfaceFileName = "ctlambda2.bin";
    //const char* cplambda3SurfaceFileName = "cplambda3.bin";
    ...
    */
    ...
    thurstLimParams->R = R;
    thurstLimParams->rho = rho;
    //thurstLimParams->ctlambda2SurfaceFileName = ctlambda2SurfaceFileName;
    thurstLimParams->ctlambda2Max = ctlambda2Max;
    thurstLimParams->ctlambda2Min = ctlambda2Min;
    thurstLimParams->minPitchMaxChangeRate = minPitchMaxChangeRate;
    thurstLimParams->samplingInterval = T;

    tsrEstParams->b = b;
    tsrEstParams->J = J;
    tsrEstParams->R = R;
    tsrEstParams->T = T;
    tsrEstParams->rho = rho;
    //tsrEstParams->cplambda3SurfaceFileName = cplambda3SurfaceFileName;
    tsrEstParams->cplambda3Max = cplambda3Max;
    tsrEstParams->cplambda3Min = cplambda3Min;
    tsrEstParams->tipSpeedRatioMax = tipSpeedRatioMax;
    ...
    /*
      ...
      H(z) = ------------------------- -----------------------------------------------------------------------------------------------------------------
              1 + T*d*w + (0.5*T*w)^2    z^2 - 2*(1 - (0.5*T*w)^2) / (1 + T*d*w + (0.5*T*w)^2)z + (1 - T*d*w + (0.5*T*w)^2) / (1 + T*d*w + (0.5*T*w)^2)
    */
    tsrEstParams->lowPass.tfParams[0].enable = 1;
    tsrEstParams->lowPass.tfParams[0].b[0] = 1.0;
    tsrEstParams->lowPass.tfParams[0].b[1] = 2.0;
    tsrEstParams->lowPass.tfParams[0].b[2] = 1.0;
    tsrEstParams->lowPass.tfParams[0].a[0] = 1.0;
    tsrEstParams->lowPass.tfParams[0].a[1] = -2 * (1 - (0.5 * T * w_lp) * (0.5 * T * w_lp)) / (1 + T * d * w_lp + (0.5 * T * w_lp) * (0.5 * T * w_lp));
    tsrEstParams->lowPass.tfParams[0].a[2] = (1 - T * d * w_lp + (0.5 * T * w_lp) * (0.5 * T * w_lp)) / (1 + T * d * w_lp + (0.5 * T * w_lp) * (0.5 * T * w_lp));

    tsrEstParams->lowPass.tfParams[1].enable = 1;
    tsrEstParams->lowPass.tfParams[1].b[0] = 1.0;
    tsrEstParams->lowPass.tfParams[1].b[1] = 2.0;
    tsrEstParams->lowPass.tfParams[1].b[2] = 1.0;
    tsrEstParams->lowPass.tfParams[1].a[0] = 1.0;
    tsrEstParams->lowPass.tfParams[1].a[1] = -2 * (1 - (0.5 * T * w_lp) * (0.5 * T * w_lp)) / (1 + T * d * w_lp + (0.5 * T * w_lp) * (0.5 * T * w_lp));
    tsrEstParams->lowPass.tfParams[1].a[2] = (1 - T * d * w_lp + (0.5 * T * w_lp) * (0.5 * T * w_lp)) / (1 + T * d * w_lp + (0.5 * T * w_lp) * (0.5 * T * w_lp));

    tsrEstParams->lowPass.tfParams[2].enable = 1;
    tsrEstParams->lowPass.tfParams[2].b[0] = ((0.5 * T * w_lp) * (0.5 * T * w_lp) / (1 + T * d * w_lp + (0.5 * T * w_lp) * (0.5 * T * w_lp))) * ((0.5 * T * w_lp) * (0.5 * T * w_lp) / (1 + T * d * w_lp + (0.5 * T * w_lp) * (0.5 * T * w_lp)));
    ...
    */
    ...
    tsrEstParams->notches.dT = T;
    tsrEstParams->notches.notchParams[0].enable = 1;
    tsrEstParams->notches.notchParams[0].freq = wfa;
    tsrEstParams->notches.notchParams[0].dampNum = dnumfa;
    tsrEstParams->notches.notchParams[0].dampDen = ddenfa;

    tsrEstParams->notches.notchParams[1].enable = 1;
    tsrEstParams->notches.notchParams[1].freq = w3p;
    tsrEstParams->notches.notchParams[1].dampNum = dnum3p;
    tsrEstParams->notches.notchParams[1].dampDen = dden3p;
}
